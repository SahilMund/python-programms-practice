...
def kishor1():
    ...
    def EmployeeInformation():
        ...
        lbl12.grid(column=4, row=4)
        ent12 = Entry(window, width=30)
        ent12.grid(column=4, row=5)

        def clicked():
            window.destroy()
            window1()

        btn = Button(window, text="Back", command=clicked).grid(column=4, row=7)
        def view():
        ...
        btn = Button(window, text="View", command=view).grid(column=8, row=5)
        window.mainloop()
